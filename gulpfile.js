...
gulp.task('font-woff', function () { //перекидываем шрифты из директории src в build, а заодно следим за новыми файлами, чтобы обновлять браузер, когда появляется шрифт
...
gulp.task('font-woff2', function () { //перекидываем шрифты из директории src в build, а заодно следим за новыми файлами, чтобы обновлять браузер, когда появляется шрифт
  return gulp.src('src/fonts/**/*.+(eot|svg|ttf|otf|woff|woff2)')
    .pipe(ttf2woff2())
    .pipe(gulp.dest('build/fonts/'))
    .pipe(browserSync.reload({
...
gulp.task('font-eot', function () { //перекидываем шрифты из директории src в build, а заодно следим за новыми файлами, чтобы обновлять браузер, когда появляется шрифт
  return gulp.src('src/fonts/**/*.+(eot|svg|ttf|otf|woff|woff2)')
    .pipe(ttf2eot())
    .pipe(gulp.dest('build/fonts/'))
    .pipe(browserSync.reload({
...
//------------------ ДАННАЯ ФУНКЦИЯ ЕЩЁ НЕ ГОТОВА --------------------

// gulp.task('font-file', function () {
//   return gulp.src('build/fonts/**/*.+(eot|svg|ttf|otf|woff|woff2)')
//     .pipe(font2css())
//     .pipe(concat('_local-fonts.scss'))
...
gulp.task('watch', function () { //Следим за изменениями в файлах и директориях и запускаем задачи, если эти изменения произошли
  ...
  gulp.watch('src/fonts/**/*.*', gulp.parallel('font-woff', 'font-woff2', 'font-eot'));
  //gulp.watch('build/fonts/**/*.*', gulp.parallel('font-file'));
...
gulp.task('default', gulp.parallel('browser-sync', 'watch', 'scss', 'style', 'script', 'minjs', 'html', 'font-woff', 'font-eot', 'font-woff2', 'images')) //запускает все перечисленные задачи разом